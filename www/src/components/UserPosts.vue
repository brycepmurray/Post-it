<template>
  <div class="body">
    <UserPostsNavbar :user="user" :posts="posts">
    </userPostsNavbar>
    ...
    <div class="d-none">{{activeUser.name}}</div>
    <!--This is required to call the user computed prop-->
  ...
  export default {
    ...
    computed: {
      ...
      user() {
        ...
        if (!this.firedRequest && this.activeUser._id) {
          this.firedRequest = true
          console.log('getting user posts', this.activeUser)
          this.$store.dispatch('getUserPosts', this.activeUser)
        }
        return this.$store.dispatch('getUserById', userId)
      },
      activeUser() {
